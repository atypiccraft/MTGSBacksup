...
try:
        ...
        def backup_db(section):
                ...
                cmd = ['mysqldump', '--add-drop-table', '-h', dbhost, '-u', user, "-p'" + password + "'", name, '>', targetdir + name + '_$(date +%y%m%d).sql']
                system(' '.join(cmd))
except:
        pass
...
def backup_domain(section):
        ...
        system('tar -zvcf '+targetdir+domain+'_$(date +%y%m%d).tar.gz '+dpath)
...
''' removing geriatric files (7 days) '''
    
ctime = time.time()
path = targetdir

days = get_value('age.days', 'How many days shall backups be stored? ')
                
for i in os.listdir(path):
        try:
                i = os.path.join(path, i)
                if os.stat(i).st_mtime <= ctime - int(days) * 86400:
                        os.remove(i)
        except OSError:
                print ('y u no work!?')
